...
- BBB can be run from 2 storage sources
    - SD Card
    - eMMC(Embedded Mutli Media Card) : a storage that directly solder onto board.
...
- Navigate to `/opt/scripts/tools/eMMC` and execute init-eMMC-flasher
...
Identify and Make sure you have a main compiled file that is ready to run by executed it via `./name_program`

In case of Simulink generated file.
...
1. Create a shell script (name.sh) in ```/usr/bin/``` and make it executable
```bash
cd /usr/bin/
sudo touch name.sh
...
2. Create service file in ```/lib/systemd/``` (if not working create in ```/lib/systemd/system/```)
```bash
cd /lib/system/
